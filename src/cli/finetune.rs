...
pub fn dispatch(argv: &[String]) -> AvcResult<()> {
    ...
    if argv.is_empty() {
        return Err(AvcError::Arg("finetune start|list|show|publish|drift ...".into()));
    }
    ...
    match argv_ref[0] {
        ...
        "drift" => {
            ...
            let fj_id = argv_ref.get(2).copied()
            ...
            while i < argv_ref.len() {
                match argv_ref[i] {
                    "--embed" => { embed_name = argv_ref.get(i+1).copied(); i += 2; }
                    "--threshold" => { threshold = argv_ref.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(threshold); i += 2; }
                    _ => { i += 1; }
                }
            }
            ...
            let target_v = target_v.ok_or_else(|| AvcError::Conflict(
                format!("finetune job '{}' has no target_version", fj_id)
            ))?;
            // 先从 DB 取 base voice embed
            let base = crate::svc::drift::fetch_voice_embed(&db, &name, base_v)?
                .ok_or_else(|| AvcError::Conflict(format!(
                    "persona '{}' version {} has no voice_embed", name, base_v
                )))?;
            let db_cosine =
            ...
            if !passed {
                return Err(AvcError::Conflict(format!(
                    "drift not passed: cosine={:?} < threshold={}", reported_cosine, threshold
                )));
            }
        }
