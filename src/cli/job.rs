...
pub fn dispatch(argv: &[String]) -> AvcResult<()> {
    ...
    match argv[0].as_str() {
        ...
        "show" => {
            let id = argv.get(1).ok_or_else(|| AvcError::Arg("job show <job_id> [--artifacts]".into()))?;
            let want_artifacts = argv.iter().any(|a| a == "--artifacts");
            if want_artifacts {
                let arts = crate::svc::render::list_artifacts(&db, id)?;
                let arr: Vec<serde_json::Value> = arts.iter().map(|a| {
                    serde_json::json!({
                        "kind": a.0,
                        "name": a.1,
                        "byte_size": a.2,
        ...
        "export" => {
            let id = argv.get(1).ok_or_else(|| AvcError::Arg("job export <job_id> --out <dir>".into()))?;
            // 找 --out 后的值
            let out_idx = argv.iter().position(|a| a == "--out")
                .ok_or_else(|| AvcError::Arg("job export: --out <dir> required".into()))?;
            let out = argv.get(out_idx + 1).ok_or_else(|| AvcError::Arg("job export: --out <dir> required".into()))?;
        ...
        "feedback" => {
            let id = argv.get(1).ok_or_else(|| AvcError::Arg("job feedback <job_id> --looks-unlike [--reason <text>]".into()))?;
            let looks_unlike = argv.iter().any(|a| a == "--looks-unlike");
            let reason = argv.iter().position(|a| a == "--reason")
